...
import kotlinx.android.synthetic.main.movie_searched.view.*
...
class SearchAdapter(public val context: Context) : PagedListAdapter<Movie, RecyclerView.ViewHolder>(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        ...
        if (viewType == MOVIE_VIEW_TYPE) {
            view = layoutInflater.inflate(R.layout.movie_searched, parent, false)
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if (getItemViewType(position) == MOVIE_VIEW_TYPE) {
            (holder as PopularMovieAdapter.MovieItemViewHolder).bind(getItem(position), context)
//            (holder as MovieItemViewHolder).bind(getItem(position), context)
    ...
    class MovieItemViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        fun bind(movie: Movie?, context: Context) {
            itemView.src_title.text = movie?.title
            itemView.src_date.text = movie?.releaseDate
            val moviePosterUrl: String = POSTER_BASE_URL + movie?.posterPath
            Glide.with(itemView.context).load(moviePosterUrl).into(itemView.search_item_img);
